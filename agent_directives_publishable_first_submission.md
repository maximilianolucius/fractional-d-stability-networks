...
8. **No arXiv references are permitted in the submitted paper.** The bibliography and in-text citations must not cite arXiv preprints, arXiv URLs, or arXiv identifiers. When a work has a peer-reviewed journal or conference version, cite that published version and its DOI/publisher metadata instead. If a result exists only as an arXiv preprint, it must not appear as a bibliographic reference in the paper; either replace it with an appropriate published source or avoid making any manuscript claim that depends on that preprint. arXiv may still be used internally during literature search and novelty auditing, but never as a final manuscript reference.
...
- **no bibliography entry or in-text citation points to arXiv, an arXiv identifier, or an arXiv URL; all cited works use published journal/conference versions or other admissible non-arXiv primary sources;**
...
21. cite arXiv preprints, arXiv identifiers, or arXiv URLs in the submitted manuscript or bibliography. If only an arXiv version of a result is available, do not cite it in the paper; replace it with a published source or remove/downgrade the dependent claim.
...
- [ ] No bibliography entry or in-text citation uses arXiv, an arXiv identifier, or an arXiv URL; published versions with DOI/publisher metadata are used instead.
