...
We solve that problem on the open strict-\(P(-A)\) stratum. Four orbit invariants,
\[
...
The contribution is an exact low-dimensional solution within generalized D-stability, not a new definition of fractional D-stability. The abstract all-diagonal forbidden-boundary framework is due to \cite{KushelPavani2022}; our contribution is the explicit elimination of the diagonal multiplier for the generic real three-dimensional Matignon problem. The symmetric slice recovers the single-cycle result of \cite{Siami2021}.
