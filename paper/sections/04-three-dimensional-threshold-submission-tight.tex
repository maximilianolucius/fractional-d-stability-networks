...
For \(\alpha>2/3\), necessity follows from Corollary~\ref{cor:strictP-interior} and Theorem~\ref{thm:C10}. Conversely, near any strict-P matrix the invariants vary continuously and remain in a compact positive box. On a sufficiently small parameter neighborhood, \(B_\beta(x)\) ranges in a common compact interval \([0,M]\). Since \(h_\alpha\) is continuous and strictly positive on this interval,
\[
m:=\min_{0\le b\le M}h_\alpha(b)>0,
\qquad
F_{\alpha,\beta}(x)\ge\frac{m}{x_1x_2x_3}.
\]
Thus all nearby minimizers lie in one compact subset of \(\Delta_2^\circ\), and \(T_\alpha(\beta)\) is locally continuous. The strict gap \(T_\alpha(\beta)-\kappa>0\) therefore persists under sufficiently small perturbations.
\end{proof}
