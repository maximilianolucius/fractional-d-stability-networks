...
\begin{figure}[t]
\centering
\includegraphics[width=.82\linewidth]{fig_threshold_ratio.pdf}
\caption{Fractional enlargement of Cain's threshold for representative invariant triples. Theorem~\ref{thm:alpha-monotonicity} gives strict decrease in \(\alpha\), and Theorem~\ref{thm:C14} gives convergence to \(T_1\) at integer order. The main ordinate is logarithmic; the inset resolves the approach to integer order on \(0.82\le\alpha\le1\). Colors, line styles, and markers redundantly identify the four triples.}
